...
def apply_filter_on_windowed_signal(windowed_sig, filter_func, fps=None, params={}):
    """
...
def apply_filter(sig, filter_func, fps=None, params={}):
    """
...
def normalizeRGB(sig):
    M=np.expand_dims(sig.mean(axis=2),axis=2)
    S=np.expand_dims(sig.std(axis=2),axis=2)
    sig=(sig-M)/S  
    return sig
...
#     Please refer to the method BVP.filters.rgb_filter_th.
#     """
